...
OPTIONAL_SECTIONS = ["certifications", "publications", "volunteer"]  # Used now!

WEAK_VERBS = ["did", "made", "helped", "worked on", "was involved in", "participated in"]
STRONG_VERBS = ["developed", "implemented", "designed", "optimized", "led", "built", "achieved"]
...
GENERIC_PHRASES = {
    ...
    "exposure to": "State your actual proficiency level",
}
...
ACHIEVEMENT_TRIGGERS = {
    ...
    r"\$\d+": "dollar amounts",
    r"\d+x": "multiples",
    r"improved|increased|reduced|saved": "actionable results",
}
...
def suggest_resume_improvements(
    ...
    # ==== OPTIONAL SECTIONS ====
    if jd_text:  # Only suggest if JD exists for context
        for section in OPTIONAL_SECTIONS:
            if not resume_data.get(section):
                # Special case for tech roles
                if "engineer" in jd_text.lower() and section == "certifications":
                    suggestions["medium"].append(
                        "Consider adding a 'certifications' section (e.g., AWS/Azure certs)"
                    )
                elif "research" in jd_text.lower() and section == "publications":
                    suggestions["medium"].append(
                        "Add 'publications' section if you have research papers"
                    )
    ...
    if jd_text:
        ...
        if missing_keywords:
            ...
            suggestions["critical"].append(
                f"Missing JD keywords: {', '.join(top_missing)}"
            )
    ...
    if "skills" in resume_data:
        ...
        if jd_text:
            ...
            if irrelevant_skills:
                suggestions["medium"].append(
                    f"Potentially irrelevant skills: {', '.join(irrelevant_skills)}"
                )

        # Skill formatting
        if all(isinstance(s, str) for s in resume_data["skills"]):
            suggestions["medium"].append(
                "Format skills with proficiency levels (e.g., 'Python (Advanced)')"
            )

    # ==== PROJECTS/EXPERIENCE DEEP CHECKS ====
                ...
                if isinstance(item, dict):
                    ...
                    if not has_achievements:
                        suggestions["high"].append(
                            f"Add quantifiable results to '{name}' section"
                        )
    ...
    if found_buzzwords:
        suggestions["low"].append(
            f"Avoid buzzwords: {', '.join(found_buzzwords[:3])}"
        )
