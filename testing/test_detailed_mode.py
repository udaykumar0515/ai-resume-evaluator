#!/usr/bin/env python3
"""
DETAILED MODE ACCURACY TEST
Tests the parsed + structured scoring used in Single Resume Evaluation tab (Hybrid)
This is the DETAILED mode for individual resume analysis
"""
import sys
import os
...
class DetailedModeAccuracyTester:
    def __init__(self, dataset_path="dataset.json"):
        ...
        self.actual_scores = []
        self.results = {}
        # Initialize scorer with DETAILED MODE config (Hybrid, like main app)
        self.scorer = ResumeScorer(method="hybrid", embedding_model="balanced")
    ...
    def calculate_detailed_score(self, resume_text, job_role):
        """Calculate score using DETAILED MODE (parsing + structured + hybrid)"""
        try:
            jd_text = self.get_job_description(job_role)
            
            # STEP 1: Parse resume using the actual parser (like main app)
            with tempfile.NamedTemporaryFile(mode='w', suffix='.txt', delete=False) as tmp_file:
                tmp_file.write(resume_text)
                tmp_file_path = tmp_file.name
            
            # Parse resume with structured data extraction
            ...
            # STEP 2: Combine structured resume data (like main app)
            combined_text = parser.combine_structured_resume(parsed_data)
            
            # STEP 3: Calculate score using hybrid method (like main app)
            results = self.scorer.get_similarity_score(jd_text, [combined_text])
            
            if results and len(results) > 0:
                similarity_score = results[0][1]
                return int(similarity_score * 100)
            else:
                print(f"❌ No results returned from scorer")
                return 50
    ...
    def run_predictions(self):
        """Run predictions using DETAILED MODE pipeline"""
        print("🔍 Running DETAILED MODE predictions...")
        print("   ✓ Resume parsing with NER")
        print("   ✓ Structured data extraction")
        print("   ✓ Section weighting")
        print("   ✓ Hybrid scoring (TF-IDF + Embeddings)")
        ...
        for i, sample in enumerate(self.data):
            ...
            try:
                predicted_score = self.calculate_detailed_score(
                    sample['resume_text'], 
                    sample['job_role']
                )
        ...
        print(f"✅ DETAILED MODE predictions completed: {successful_predictions} successful, {failed_predictions} failed")
    ...
    def print_results(self):
        ...
        print("\n" + "="*70)
        print("🎯 AI RESUME EVALUATOR - DETAILED MODE ACCURACY TEST")
        print("   (Parsing + Structured + Hybrid - Used in Single Resume Evaluation)")
        print("="*70)
        ...
        if r2 > 0.8 and f1 > 0.8:
            print("🟢 EXCELLENT: Detailed mode performs very well")
        elif r2 > 0.6 and f1 > 0.6:
            print("🟡 GOOD: Detailed mode performs reasonably well")
        elif r2 > 0.4 and f1 > 0.4:
            print("🟠 FAIR: Detailed mode needs improvement")
        else:
            print("🔴 POOR: Detailed mode needs significant improvement")
        
        print(f"\n📋 IMPROVEMENT RECOMMENDATIONS:")
        if r2 < 0.6:
            print("• Improve parsing accuracy and section extraction")
            print("• Better section weighting algorithms")
            print("• Enhanced hybrid scoring combination")
        if f1 < 0.6:
            print("• Better job-resume matching logic")
            print("• More sophisticated NLP models")
            print("• Improved entity recognition")
    ...
    def create_visualizations(self):
        ...
        fig.suptitle('AI Resume Evaluator - DETAILED MODE Performance Analysis\n(Parsing + Structured + Hybrid)', fontsize=16)
        ...
        plt.tight_layout()
        plt.savefig('detailed_mode_performance.png', dpi=300, bbox_inches='tight')
        print("✅ Visualizations saved to detailed_mode_performance.png")
        plt.show()
    
    def save_detailed_results(self, output_path="detailed_mode_results.csv"):
    ...
    def run_complete_test(self):
        """Run the complete DETAILED MODE testing pipeline"""
        print("🚀 Starting AI Resume Evaluator DETAILED MODE Accuracy Test")
        print("   Testing the complete pipeline used in Single Resume Evaluation")
        print("="*70)
        ...
        print("\n🎉 DETAILED MODE testing completed successfully!")
        print("📁 Check the 'testing' folder for all output files")

if __name__ == "__main__":
    # Run the complete test
    tester = DetailedModeAccuracyTester()
