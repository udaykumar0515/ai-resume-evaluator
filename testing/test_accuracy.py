# AI Resume Evaluator - Accuracy Testing Framework
import json
import pandas as pd
import numpy as np
...
class ResumeEvaluatorTester:
    def __init__(self, dataset_path="dataset.json"):
        ...
        self.actual_scores = []
        self.results = {}
        # Initialize your real scoring system
        self.resume_matcher = ResumeScorer(method="hybrid", embedding_model="balanced")
    ...
    def convert_to_csv(self, output_path="dataset.csv"):
    ...
    def parse_resume_text(self, resume_text):
        """Parse resume text using the project's parser"""
        try:
            ...
            # Parse using the project's parser with txt file type
    ...
    def calculate_real_score(self, parsed_data, job_role, resume_text):
        """Calculate score using your actual ResumeScorer system"""
        try:
            # Get full job description
            jd_text = self.get_job_description(job_role)
            
            # Use your real scoring system
            if parsed_data and parsed_data.get('raw_text'):
                # Use the parsed resume data - combine it properly
                resume_data = {
                ...
                # Calculate similarity score using your ResumeScorer
                # First combine the structured data
                from modules.parser import combine_structured_resume
                combined_text = combine_structured_resume(resume_data)
                scores = self.resume_matcher.get_similarity_score(
                    jd_text=jd_text,
                    resume_texts=[combined_text]
                )
                
                if scores and len(scores) > 0:
                    # Convert similarity score (0-1) to percentage (0-100)
                    similarity_score = scores[0][1]  # Get the score from the tuple
                ...
                if scores and len(scores) > 0:
                    similarity_score = scores[0][1]
                    return int(similarity_score * 100)
                else:
                    return 50
                    
        except Exception as e:
            print(f"❌ Error in real scoring: {e}")
            return 50  # Default score on error
    
    def run_predictions(self):
        """Run predictions on all samples using your real scoring system"""
        print("🔍 Running predictions using your real ResumeScorer system...")
        self.predictions = []
        self.actual_scores = []
        ...
        for i, sample in enumerate(self.data):
            ...
            try:
                # Parse the resume
                parsed_data = self.parse_resume_text(sample['resume_text'])
                
                # Calculate score using your real ResumeScorer
                predicted_score = self.calculate_real_score(
                    parsed_data, 
                    sample['job_role'], 
                    sample['resume_text']
                )
                ...
                # Use fallback score
                self.predictions.append(50)  # Default score
                self.actual_scores.append(sample['match_score'])
                failed_predictions += 1
        
        print(f"✅ Predictions completed: {successful_predictions} successful, {failed_predictions} failed")
    ...
    def calculate_metrics(self):
        ...
        # Convert scores to categories: 0-60 (Poor), 60-75 (Fair), 75-90 (Good), 90-100 (Excellent)
    ...
    def print_results(self):
        ...
        print("\n" + "="*60)
        print("🎯 AI RESUME EVALUATOR - ACCURACY TEST RESULTS")
        print("="*60)
        ...
        print(f"\n📋 RECOMMENDATIONS:")
        if r2 < 0.6:
            print("• Improve feature extraction from resumes")
            print("• Add more sophisticated scoring algorithms")
        if f1 < 0.6:
            print("• Better job-resume matching logic")
            print("• Consider using more advanced NLP models")
    ...
    def create_visualizations(self):
        ...
        fig.suptitle('AI Resume Evaluator - Performance Analysis', fontsize=16)
    ...
    def save_detailed_results(self, output_path="detailed_results.csv"):
    ...
    def run_complete_test(self):
        ...
        print("🚀 Starting AI Resume Evaluator Accuracy Test")
        print("="*60)
